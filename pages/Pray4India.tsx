...
const App: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-x-hidden selection:bg-orange-100 dark:selection:bg-orange-900 selection:text-orange-900 dark:selection:text-orange-100 transition-colors duration-500">
                            ...
                            <AnimatePresence mode="wait">
                                {formStatus === 'success' ? (
                                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-10">
                                        ...
                                        <p className="text-xl md:text-base lg:text-[11px] text-slate-500 dark:text-slate-400 mb-12 max-w-lg mx-auto">{t.join.successDesc}</p>
                                    ...
                                    <form onSubmit={handleJoinSubmit} className="space-y-6 md:space-y-10">
                                        <div className="grid sm:grid-cols-2 gap-6 md:gap-10">
                                            <div className="space-y-4">
                                                <label className="text-sm lg:text-[11px] font-bold text-slate-500 uppercase tracking-widest">{t.join.langLabel}</label>
                                                <select className="w-full px-5 py-4 md:px-6 md:py-3 rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold uppercase md:text-sm lg:text-[11px]" value={selectedLang} onChange={(e) => setSelectedLang(e.target.value as Language)} required>
                                                    {LANGUAGES.map(lang => (
                                            ...
                                            <div className="space-y-4">
                                                <label className="text-sm lg:text-[11px] font-bold text-slate-500 uppercase tracking-widest">{t.join.phoneLabel}</label>
                                                <input type="tel" placeholder="+91 00000 00000" className={`w-full px-5 py-4 md:px-6 md:py-3 rounded-2xl border-2 bg-slate-50 dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold md:text-sm lg:text-[11px] ${formErrors.phone ? 'border-red-500' : 'border-slate-100 dark:border-slate-800'}`} value={phone} onChange={(e) => setPhone(e.target.value)} required />
                                            </div>
                                        </div>
                                        <div className="space-y-4">
                                            <label className="text-sm lg:text-[11px] font-bold text-slate-500 uppercase tracking-widest">{t.join.emailLabel}</label>
                                            <input type="email" placeholder="you@example.com" className={`w-full px-5 py-4 md:px-6 md:py-3 rounded-2xl border-2 bg-slate-50 dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold md:text-sm lg:text-[11px] ${formErrors.email ? 'border-red-500' : 'border-slate-100 dark:border-slate-800'}`} value={email} onChange={(e) => setEmail(e.target.value)} required />
                                        </div>
                                        <button type="submit" disabled={formStatus === 'submitting'} className="w-full py-4 md:py-6 lg:py-4 bg-orange-600 text-white font-bold rounded-2xl hover:bg-orange-700 transition-all shadow-xl flex items-center justify-center gap-2 text-lg md:text-sm uppercase tracking-widest">
            ...
            <section id="contact" className="py-8 md:py-24 bg-white dark:bg-slate-950">
                <div className="max-w-7xl mx-auto px-4 sm:px-6">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900 p-4 sm:p-6 md:p-12 lg:p-20 rounded-xl md:rounded-[4rem] border dark:border-slate-800 shadow-2xl">
                            <form onSubmit={handleContactSubmit} className="space-y-4 md:space-y-10">
                                <div className="space-y-1.5">
                                    ...
                                    <input type="text" className="w-full px-3 py-2 md:px-6 md:py-3 rounded-lg md:rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold text-xs lg:text-[11px]" value={contactName} onChange={(e) => setContactName(e.target.value)} required />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t.contact.email}</label>
                                    <input type="email" className={`w-full px-3 py-2 md:px-6 md:py-3 rounded-lg md:rounded-xl border bg-white dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold text-xs lg:text-[11px] ${contactErrors.email ? 'border-red-500' : 'border-slate-200 dark:border-slate-800'}`} value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} required />
                                </div>
                                <textarea rows={3} placeholder={t.contact.message} className="w-full px-3 py-2 md:px-6 md:py-3 rounded-lg md:rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold resize-none text-xs lg:text-[11px]" value={contactMessage} onChange={(e) => setContactMessage(e.target.value)} required></textarea>
                                <button type="submit" disabled={contactStatus === 'submitting'} className="w-full py-2.5 md:py-5 lg:py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-950 font-bold rounded-lg md:rounded-xl hover:bg-slate-800 dark:hover:bg-slate-100 transition-all flex items-center justify-center gap-2 text-xs md:text-sm lg:text-[11px] uppercase tracking-widest">
