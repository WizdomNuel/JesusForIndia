    ...
    <div className="flex flex-col min-h-screen pt-40 pb-24">
      ...
      <section className="mb-24 text-center reveal px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-5xl sm:text-8xl font-black text-slate-800 serif leading-none mb-8">
            Stories <span className="text-orange-600 italic">of Grace</span>
          </h1>
          <p className="text-slate-500 text-lg sm:text-xl serif max-w-2xl mx-auto italic">Capturing the moments where faith meets action across the vibrant landscape of India.</p>
        </div>
      </section>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {['ALL', 'VILLAGE_LIFE', 'SCRIPTURE', 'OUTREACH', 'WORSHIP', 'COMMUNITY'].map((cat) => (
            <button
              ...
              className={`px-8 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all duration-500 ${activeCategory === cat ? 'bg-slate-800 text-white shadow-lg' : 'bg-white text-slate-500 hover:text-slate-900 border border-slate-100'}`}
            >
              {cat.replace('_', ' ')}
            </button>
          ))}
      ...
      <section className="max-w-7xl mx-auto px-6 reveal mb-32">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {stories.filter(s => activeCategory === 'ALL' || s.category === activeCategory).map((story) => (
            <div 
              key={story.id} 
              className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-700 group cursor-pointer border border-slate-50"
            ...
            >
              ...
              <div className="p-10">
                ...
                <h4 className="text-2xl font-black text-slate-800 serif group-hover:text-orange-700 transition-colors leading-tight mb-4">{story.title}</h4>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div className="reveal">
              ...
              <h2 className="text-4xl sm:text-6xl font-black text-white serif mb-8">Film <span className="italic text-orange-500">Highlights</span></h2>
            ...
            <div className="reveal">
               <div className="aspect-video bg-slate-800 rounded-[3rem] overflow-hidden shadow-2xl relative group cursor-pointer border-4 border-slate-700">
      ...
      {selectedStory && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white w-full max-w-5xl rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-500">
            <button onClick={() => setSelectedStory(null)} className="absolute top-8 right-8 z-10 w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-orange-100 hover:text-orange-700 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="p-12 sm:p-20 flex flex-col justify-center">
                ...
                <h2 className="text-4xl sm:text-6xl font-black text-slate-800 serif leading-none mb-8">{selectedStory.title}</h2>
                <p className="text-slate-500 text-lg leading-relaxed mb-10 italic serif">
                ...
                <div className="flex space-x-4">
                  <button className="bg-slate-800 text-white px-10 py-4 rounded-full text-xs font-bold uppercase tracking-widest w-fit">Download Report</button>
                  <button onClick={() => setSelectedStory(null)} className="border border-slate-200 px-10 py-4 rounded-full text-xs font-bold uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-all">Close</button>
                </div>
              </div>
            </div>
          </div>
