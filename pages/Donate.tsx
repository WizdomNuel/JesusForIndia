    ...
    <div className="flex flex-col min-h-screen pt-24 sm:pt-40 pb-24 dark:bg-slate-900 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-6 w-full">
        ...
        <div className="text-center mb-24 reveal">
          ...
          <h1 className="text-4xl sm:text-8xl font-black text-slate-800 dark:text-slate-100 serif tracking-tight mb-8">
            Partner in <span className="text-orange-600 italic">Faith</span>
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-base sm:text-xl serif max-w-2xl mx-auto italic">Your support directly empowers local missions to bring hope and restoration to unreached hearts across India.</p>
        </div>
        ...
        <section className="mb-32 reveal">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="bg-white dark:bg-slate-800 p-8 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] border border-slate-50 dark:border-slate-700 shadow-sm text-center group hover:shadow-xl transition-all duration-500">
                <div className="text-3xl sm:text-4xl mb-6 group-hover:scale-125 transition-transform">{gift.icon}</div>
                <div className="text-2xl sm:text-3xl font-black serif text-orange-600 mb-2">{gift.amt}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 items-start mb-32">
          ...
          <div className="space-y-12 reveal">
            <div className="bg-white dark:bg-slate-800 p-8 sm:p-20 rounded-[2.5rem] sm:rounded-[3.5rem] shadow-sm border border-slate-50 dark:border-slate-700 relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-64 h-64 bg-orange-50 dark:bg-orange-900/10 rounded-full blur-[80px] pointer-events-none"></div>
          ...
          <div id="donation-card" className="bg-white dark:bg-slate-800 p-8 sm:p-20 rounded-[2.5rem] sm:rounded-[4rem] shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 reveal" style={{ transitionDelay: '0.3s' }}>
            <h3 className="text-2xl sm:text-4xl font-black text-slate-800 dark:text-slate-100 mb-12 serif text-center">Support the Call</h3>

            <div className="flex p-2 bg-slate-50 dark:bg-slate-900 rounded-full mb-12 border border-slate-200/50 dark:border-slate-700">
              <button onClick={() => setIsMonthly(false)} className={`flex-1 py-4 rounded-full font-bold text-[10px] uppercase tracking-widest transition-all ${!isMonthly ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-md' : 'text-slate-400 dark:text-slate-500'}`}>One-Time</button>
              <button onClick={() => setIsMonthly(true)} className={`flex-1 py-4 rounded-full font-bold text-[10px] uppercase tracking-widest transition-all ${isMonthly ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-md' : 'text-slate-400 dark:text-slate-500'}`}>Monthly</button>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-12">
              {amounts.map((amt) => (
                <button key={amt} onClick={() => { setAmount(amt); setCustomAmount(''); }} className={`py-6 sm:py-8 rounded-2xl sm:rounded-3xl font-black text-xl sm:text-2xl serif transition-all ${amount === amt && !customAmount ? 'bg-orange-600 text-white shadow-xl shadow-orange-100' : 'bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 border border-transparent hover:border-orange-100'}`}>
                  ${amt}
                </button>
              ))}
            </div>
            ...
            <div className="mb-12 relative">
              <input
                ...
                className="w-full bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-700 rounded-3xl px-6 sm:px-10 py-6 sm:py-10 text-3xl sm:text-5xl font-black text-center text-slate-800 dark:text-white focus:outline-none focus:border-orange-500 transition-all serif"
                placeholder="0.00"
              />
              <span className="absolute top-4 left-1/2 -translate-x-1/2 text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">Custom Amount</span>
            </div>
        ...
        <section className="bg-green-800 dark:bg-green-950 p-8 sm:p-32 rounded-[2.5rem] sm:rounded-[4rem] text-white relative overflow-hidden reveal">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
              <h3 className="text-3xl sm:text-6xl font-black serif leading-tight mb-8">Join the Monthly <span className="italic text-orange-400">Mission Circle</span></h3>
              <p className="text-green-100 text-lg sm:text-xl serif leading-relaxed italic mb-12">
